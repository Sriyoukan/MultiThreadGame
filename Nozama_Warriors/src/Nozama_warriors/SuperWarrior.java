...
public class SuperWarrior extends Warrior implements HumanAction,Runnable {
    ...
    public void swim() {
        ...
        while (Grid.notreached) {
            ...
            if (oldx >= 5 && oldy >= 5) {                                                                               // Checking the warrior's node
                randomno = randomGenerator.nextInt(2);                                                            // Taking 2 random numbers
                if (randomno == 0) {
                    /* In random increasing  X-1*/
                    if ((oldx - 1) > -1) {                                                                              //(x-1,y)
                        if (!(lake.allowSwim(this, oldx, oldy, oldx - 1, oldy))) {                              // tell Grid that Super Warrior is Swimming
                            break;                                                                                      //if swam it will try to swim again
                        }
                    }
                } else if (randomno == 1) {
                    /* In random increasing  Y-1*/
                    if ((oldy - 1) > -1) {                                                                              //(x,y-1)
                        if (!lake.allowSwim(this, oldx, oldy, oldx, oldy - 1)) {
                            break;
                        }
                    }
                }
                ...
                if (randomno == 0) {
                    /* In random increasing  X-1*/
                    if ((oldx - 1) > -1) {                                                                              //(x-1,y)
                        if (!lake.allowSwim(this, oldx, oldy, oldx - 1, oldy)) {
                            break;
                        }
                    }
                } else if (randomno == 1) {
